...
app.post('/home/pick_color', function(req, res) {
	...
	var insert_statement = 'INSERT INTO favorite_colors(hex_value, name, color_msg) VALUES ($1,$2,$3);'// Write a SQL statement to insert a color into the favorite_colors table
...
app.get('/player_info/select_player', function(req, res){
	...
	db.task('load-players', task =>
		...
		.then(results => {
			...
			var games = null;
			if(selectedPlayer){
				games = task.one('SELECT COUNT(*) AS games_played FROM football_games WHERE $1 = ANY(players)', selectedPlayer.id);
			}
			return Promise.all([Promise.resolve(results[0]), Promise.resolve(selectedPlayer), games]);
		})
		.catch(err => {
			console.log('error', err);
		})
	...
	.then(info => {
		...
		selected_player.games_played = info[2].games_played;
...
app.post('/player_info/add_player', function(req, res) {
	...
	var p_yards = req.body.player_passing_yards;
	var rush_yards = req.body.player_rushing_yards;
	var receiving_yards = req.body.player_receiving_yards;
	var insert_statement = "insert into football_players(name, year, major, passing_yards, rushing_yards, receiving_yards) values($1, $2, $3, $4, $5, $6);";
	var player_select = 'select * from football_players;';
	db.task('get-everything', task => {
		return task.batch([
			task.any(insert_statement, [name, year, major, p_yards, rush_yards, receiving_yards]),
